...
作者：fenghuohuo
日期：2021年11月16日
注： 三点法的误差可能比较大 1.多次求三点，取均值 在之前可以进行数据处理
"""
###########################################################################
# 由圆上三点确定圆心和半径
###########################################################################
...
def points2circle(p1, p2, p3):
    ...
    temp01 = p1 - p2
    temp02 = p3 - p2
    temp03 = np.cross(temp01, temp02)       # 向量叉乘
    temp = (temp03 @ temp03) / (temp01 @ temp01) / (temp02 @ temp02)    # @是矩阵乘法
    if temp < 10**-6:
        print('\t三点共线, 无法确定圆')
        return None
