...
class Msrcr():
    ...
    def multiScaleRetinexWithColorRestorationProcess(self, img, sigma_list, G, b, alpha, beta):
    ...
    def SSR(self, img, sigma=300):
        ssr = self.singleScaleRetinexProcess(img, sigma)
        ssr = self.touint8(ssr)
        return ssr

    def MSR(self, img, sigma_list=[15, 80, 250]):
        msr = self.multiScaleRetinexProcess(img, sigma_list)
        msr = self.touint8(msr)
        return msr

    def MSRCR(self, img, sigma_list=[15, 80, 250], G=5, b=25, alpha=125, beta=46, low_clip=0.01, high_clip=0.99):
        msrcr = self.multiScaleRetinexWithColorRestorationProcess(img, sigma_list, G, b, alpha, beta)
